...
export default function Card({ QueryData }) {
  ...
  const FetchData = async () => {
    const Response = await getApiData(page, QueryData);
    setData(Response);
  };
  useEffect(() => {
    FetchData();
  ...
  return (
    <>
      <div id="container">
        ...
        <div id="pagechange">
          {page == 1 || (
            <button className="npbutton1 nepr" onClick={previous}>
              &larr;Previous
            </button>
          )}
          <button className="npbutton2  nepr" onClick={next}>
            Next &rarr;{" "}
          </button>
        </div>
      </div>
